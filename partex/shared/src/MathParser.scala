...
object MathParser{
  ...
  def identifier[_:P]: P[Identifier] = P(CharIn("a-zA-Z").! ~ symAttr.rep.map(_.toVector) ~ (&("\\")|ws.rep))
  ...
  def paren[_:P]: P[Expr] = P("(" ~ expr ~ ")" ~ symAttr.rep.map(_.toVector))
  ...
  def subExpr[_:P]: P[Subscript] = P("_{" ~ expr ~ "}").map((e: Expr) => Subscript(e))
  def superExpr[_:P]: P[Superscript] = P("^{" ~ expr ~ "}").map((e: Expr) => Superscript(e))
  def subscript[_:P]: P[Subscript] = P("_" ~ singleChar).map((e: Expr) => Subscript(e))
  def superscript[_:P]: P[Superscript] = P("^" ~ singleChar).map((e: Expr) => Superscript(e))
